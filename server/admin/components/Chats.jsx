...
class Chats extends React.Component {
    ...
    sendMessage(sessionHash, message, selector) {
        let that    =   this;
        ajax.post(`${appConfig.URL}/chat/message/${sessionHash}`)
            ...
            .end(() => {that.props.getChats(that.props.state.login.data.agent[0].company_id, that.props.chatHashesChoosed)});

    }

    // shouldComponentUpdate(nextProps, nextState) {
    //     let that    =   this;
    //     if(this.props.state.chat.body && this.props.state.chat.body.messages.data && nextProps.state.chat.body.messages && nextProps.state.chat.body.messages.data) {
    //         for(let i in this.props.state.chat.body.messages.data) {
    //             if( nextProps.state.chat.body.messages.data[i] && this.props.state.chat.body.messages.data[i].length != nextProps.state.chat.body.messages.data[i].length) {
    //                 setTimeout(() => {that.refs['messages_' + i].scrollTop = that.refs['messages_' + i].scrollHeight;});
    //                 break;
    //             }
    //         }
    //     }
                        ...
                        <Card key={i} className="col-sm-2">
                            ...
                            <div className="chat-text">
                                <div className="col-sm-8">
                                    <textarea onChange={(event) => {this.setLastMessageText(event, sessionKeys[i])}} className="form-control chat-text"  rows="2"></textarea>
                                </div>
                                <div className="col-sm-4">
                                    <RaisedButton className="send-button" label={<CommunicationChat />} primary={true}  onClick={() => {this.sendMessage(sessionKeys[i], this.state.lastMessageText[sessionKeys[i]], this)}} />
                                </div>
                            </div>
                        </Card>
                ))
